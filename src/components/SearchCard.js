...
import React from 'react';
import stock from '../apis/stock'; // Your configured Axios instance
import '../css/SearchCard.css';
import '../css/styles.css';
import { FaSearch } from 'react-icons/fa';
...
class SearchCard extends React.Component {
    ...
    componentDidMount() {
        ...
        const lsArray = localStorage.getItem("historyStockArray");
        if (lsArray !== null) {
            if (window.confirm("You have saved stocks. Would you like to load them?")) {
                JSON.parse(lsArray).forEach(stock => this.sendSearchResult(false, stock));
            } else {
                localStorage.removeItem("historyStockArray");
            }
        }
    ...
    componentDidUpdate() {
        if (document.querySelector(".stock-code__value").value === '') {
            document.querySelector(".btn-search").disabled = true;
        }
    }
    ...
    sendSearchResult = async (torf, value) => {
        ...
        try {
            ...
            if (table_response.data && table_response.data.c !== undefined) {
                this.setState(prevState => ({
                    search_stockArray: [...prevState.search_stockArray, stockValue],
                    loading_api: false,
                }), () => {
                    if (table_response.data.c === 0) {
                        this.props.sendSearchGraphResult("no_data", '');
                    } else {
                ...
                alert("Stock not found or invalid response.");
                this.setState({ loading_api: false });
            }
        } catch (error) {
            console.error('Error fetching stock data:', error);
            alert("Failed to fetch stock data. Please check your stock code and try again.");
            this.setState({ loading_api: false });
        }
    };

    validateBtn = (val) => {
        document.querySelector(".btn-search").disabled = val === '' || val.length > 4;
    };
            ...
            <div className="card card-container search">
                <div className="card-body">
                    ...
                    <button 
                        ...
                        disabled={this.state.loading_api}
                    >
